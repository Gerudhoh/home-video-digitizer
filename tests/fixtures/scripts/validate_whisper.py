...
def normalize_whisper_transcript(whisper_transcript_path):
    whisper_json = json.loads(Path(whisper_transcript_path).read_text(encoding="utf-8"))
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print(
            "Incorrect arguments! Please try again with: validate_whisper.py <whisper transcript path> <golden transcript path>"
        )
        sys.exit(1)
