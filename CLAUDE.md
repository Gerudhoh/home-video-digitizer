...
Most of the pipeline is still planning-stage, but the **Transcribe** stage is implemented: `scripts/transcribe.py` extracts audio with ffmpeg, ships it to a Whisper server running on the homelab (bespin LXC) over SMB, and returns `{language, duration, text}`. It currently does *not* produce `source`, `recorded_date`, `segments`, or `tags` — see "Transcript schema" below for what's aspirational vs. actually emitted today. A test harness for judging transcription accuracy against hand-written goldens also exists under `tests/fixtures/` (LLM-as-judge via Ollama; see `issues/plans/002-transcript-testing.md`). Deterministic scene-detection regression tests also exist for the ffmpeg/PySceneDetect step (`tests/fixtures/tests/test_scene_split.py`), covering #12's "clean split at a normal boundary" and "silence/low-narration segment is NOT incorrectly split" cases against small clips checked in under `tests/fixtures/raw/christian/scene_split/` (trimmed from the gitignored, 991MB source tape — regenerate with `python tests/fixtures/scripts/make_scene_split_fixtures.py` if they go missing or the source tape changes); #12's date-boundary and transcript-realignment cases are out of scope since most tapes carry no burned-in date. The rest of the architecture below is still the *planned* design, pulled from GitHub issues, not yet implemented. There are no build/lint commands yet; `pytest` (or `rtk pytest`) runs the deterministic suite by default. The whisper-transcript-quality tests are LLM-judged and nondeterministic, so they're marked `quality` and excluded unless run explicitly via `pytest -m quality` (also needs a reachable Ollama/Whisper instance, or they skip).
